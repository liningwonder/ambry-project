...
public class NettyRequest implements RestRequest {
  ...
  public NettyRequest(HttpRequest request, Channel channel, NettyMetrics nettyMetrics,
    ...
    if (nettyCookies != null) {
      Set<Cookie> cookies = convertHttpToJavaCookies(nettyCookies);
   ...
   * @return Set of {@link Cookie}s equivalent to the passed in
  ...
  private Set<Cookie> convertHttpToJavaCookies(Set<io.netty.handler.codec.http.cookie.Cookie> httpCookies) {
    ...
    for (io.netty.handler.codec.http.cookie.Cookie cookie : httpCookies) {
      try {
        Cookie javaCookie = new Cookie(cookie.name(), cookie.value());
  ...
  /**
   * The {@link NettyRequest}'s implementation of {@link RestRequestContext}.
