...
class AmbrySecurityService implements SecurityService {
  ...
  public void processResponse(RestRequest restRequest, RestResponseChannel responseChannel, BlobInfo blobInfo,
    ...
    if (!isOpen) {
      ...
      try {
        ...
        responseChannel.setHeader(Headers.DATE, new GregorianCalendar().getTime());
        switch (restMethod) {
          case HEAD:
            ...
            responseChannel.setHeader(Headers.LAST_MODIFIED,
          ...
          case GET:
            if (!requestPath.matchesOperation(Operations.GET_SIGNED_URL)) {
              ...
              SubResource subResource = RestUtils.getRequestPath(restRequest).getSubResource();
              responseChannel.setHeader(Headers.LAST_MODIFIED,
                  new Date(blobInfo.getBlobProperties().getCreationTimeInMs()));
              if (subResource == null) {
                ...
                if (ifModifiedSinceMs != null
                  ...
                  responseChannel.setHeader(Headers.LIFE_VERSION, blobInfo.getLifeVersion());
                }
                setCacheHeaders(restRequest, responseChannel);
              } else {
                if (subResource.equals(SubResource.BlobInfo)) {
                  setBlobPropertiesHeaders(blobInfo.getBlobProperties(), responseChannel);
                  setAccountAndContainerHeaders(restRequest, responseChannel);
                  responseChannel.setHeader(Headers.LIFE_VERSION, blobInfo.getLifeVersion());
                }
          ...
          case POST:
            ...
            responseChannel.setHeader(Headers.CREATION_TIME,
                new Date(blobInfo.getBlobProperties().getCreationTimeInMs()));
            break;
          case OPTIONS:
          case PUT:
  ...
  /**
   * Fetches the {@link Headers#IF_MODIFIED_SINCE} value in epoch time if present
   * @param restRequest the {@link RestRequest} that needs to be parsed
   * @return the {@link Headers#IF_MODIFIED_SINCE} value in epoch time if present
   */
  private Long getIfModifiedSinceMs(RestRequest restRequest) {
    if (restRequest.getArgs().get(Headers.IF_MODIFIED_SINCE) != null) {
      return RestUtils.getTimeFromDateString((String) restRequest.getArgs().get(Headers.IF_MODIFIED_SINCE));
    }
    return null;
  }
  ...
  private void setHeadResponseHeaders(BlobInfo blobInfo, GetBlobOptions options, RestRequest restRequest,
    ...
    if (blobProperties.getContentType() != null) {
      restResponseChannel.setHeader(Headers.CONTENT_TYPE, blobProperties.getContentType());
    }
    restResponseChannel.setHeader(Headers.ACCEPT_RANGES, RestUtils.BYTE_RANGE_UNITS);
    ...
    if (options.getRange() != null) {
      ...
      restResponseChannel.setHeader(Headers.CONTENT_RANGE, rangeAndLength.getFirst());
      contentLength = rangeAndLength.getSecond();
    }
    restResponseChannel.setHeader(Headers.CONTENT_LENGTH, contentLength);
    setBlobPropertiesHeaders(blobProperties, restResponseChannel);
    setAccountAndContainerHeaders(restRequest, restResponseChannel);
    restResponseChannel.setHeader(Headers.LIFE_VERSION, blobInfo.getLifeVersion());
  }
  ...
  private void setGetBlobResponseHeaders(BlobInfo blobInfo, GetBlobOptions options,
      RestResponseChannel restResponseChannel) throws RestServiceException {
    BlobProperties blobProperties = blobInfo.getBlobProperties();
    restResponseChannel.setHeader(Headers.BLOB_SIZE, blobProperties.getBlobSize());
    restResponseChannel.setHeader(Headers.ACCEPT_RANGES, RestUtils.BYTE_RANGE_UNITS);
    long contentLength = blobProperties.getBlobSize();
    if (options.getRange() != null) {
      Pair<String, Long> rangeAndLength = RestUtils.buildContentRangeAndLength(options.getRange(), contentLength);
  ...
  private void setCacheHeaders(RestRequest restRequest, RestResponseChannel restResponseChannel)
    ...
    if (container.isCacheable()) {
      restResponseChannel.setHeader(Headers.EXPIRES,
          new Date(System.currentTimeMillis() + frontendConfig.cacheValiditySeconds * Time.MsPerSec));
      restResponseChannel.setHeader(Headers.CACHE_CONTROL, "max-age=" + frontendConfig.cacheValiditySeconds);
    } else {
      restResponseChannel.setHeader(Headers.EXPIRES, restResponseChannel.getHeader(Headers.DATE));
      restResponseChannel.setHeader(Headers.CACHE_CONTROL, "private, no-cache, no-store, proxy-revalidate");
      restResponseChannel.setHeader(Headers.PRAGMA, "no-cache");
    }
  }
  ...
  private void setBlobPropertiesHeaders(BlobProperties blobProperties, RestResponseChannel restResponseChannel)
      throws RestServiceException {
    restResponseChannel.setHeader(Headers.BLOB_SIZE, blobProperties.getBlobSize());
    restResponseChannel.setHeader(Headers.SERVICE_ID, blobProperties.getServiceId());
    restResponseChannel.setHeader(Headers.CREATION_TIME, new Date(blobProperties.getCreationTimeInMs()));
    restResponseChannel.setHeader(Headers.ENCRYPTED_IN_STORAGE, blobProperties.isEncrypted());
    if (blobProperties.getTimeToLiveInSeconds() != Utils.Infinite_Time) {
      restResponseChannel.setHeader(Headers.TTL, Long.toString(blobProperties.getTimeToLiveInSeconds()));
    }
    if (blobProperties.getContentType() != null) {
      restResponseChannel.setHeader(Headers.AMBRY_CONTENT_TYPE, blobProperties.getContentType());
    }
    if (blobProperties.getOwnerId() != null) {
      restResponseChannel.setHeader(Headers.OWNER_ID, blobProperties.getOwnerId());
    }
  }
  ...
  private void setAccountAndContainerHeaders(RestRequest restRequest, RestResponseChannel restResponseChannel)
    ...
    if (account.getId() != Account.UNKNOWN_ACCOUNT_ID) {
      restResponseChannel.setHeader(Headers.TARGET_ACCOUNT_NAME, account.getName());
      restResponseChannel.setHeader(Headers.TARGET_CONTAINER_NAME, container.getName());
    }
    restResponseChannel.setHeader(Headers.PRIVATE, !container.isCacheable());
  }
   ...
   * @param expectSecurePath the expected secure path specified in {@link FrontendConfig}.
   * @throws RestServiceException
   */
