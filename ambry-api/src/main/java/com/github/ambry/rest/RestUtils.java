...
public class RestUtils {
   ...
   * @throws RestServiceException if the {@link Headers#GET_OPTION} is present but not recognized.
   */
  public static GetOption getGetOption(RestRequest restRequest, GetOption defaultGetOption)
    ...
    Object value = restRequest.getArgs().get(Headers.GET_OPTION);
    if (value != null) {
      ...
      if (!foundMatch) {
        throw new RestServiceException("Unrecognized value for [" + Headers.GET_OPTION + "]: " + str,
            RestServiceErrorCode.InvalidArgs);
      }
    }
    return option;
